...
def get_paths(path, name='coco', use_restval=False):
    ...
    if 'coco' == name:
        ...
        imgdir = os.path.join('/media/vishwash/Data/data/crawled_data/csd', 'images')
        capsdir = 'caps.txt'
        roots['train'] = {'img': imgdir+'', 'cap': cap}
        roots['val'] = {'img': imgdir, 'cap': cap}
        roots['test'] = {'img': imgdir, 'cap': cap}
        ...
        imgdir = os.path.join('/home/dcsaero01/data/projects/DataFetcher', 'images')
        capsdir = '/home/dcsaero01/data/projects/DataFetcher/vsepp_data/train_captions.txt'
        capsdir2 = '/home/dcsaero01/data/projects/DataFetcher/vsepp_data/test_captions.txt'
        roots['train'] = {'img': imgdir, 'cap': capsdir}
        roots['val'] = {'img': imgdir, 'cap': capsdir2}
        roots['test'] = {'img': imgdir, 'cap': capsdir2}
        ids = {'train': '/home/dcsaero01/data/projects/DataFetcher/vsepp_data/train.txt',
         'val': '/home/dcsaero01/data/projects/DataFetcher/vsepp_data/test.txt',
          'test': '/home/dcsaero01/data/projects/DataFetcher/vsepp_data/test'}
    return roots, ids
...
class MiniCSDV2Dataset(data.Dataset):
    """COCO Custom Dataset compatible with torch.utils.data.DataLoader."""
    ...
    def __init__(self, root, json, vocab, transform=None, ids=None):
        ...
        with open(ids,'rb') as f:
            for line in f:
                self.imgnames.append(line.strip()[1:-1])
    ...
    def get_raw_item(self, index):
        ...
        root = self.root
        path = self.imgnames[index]
        caption = self.captions[index]
        img_id = int(path.split('_')[0]+path.split('_')[1].split('.')[0])
...
class PrecompDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Convert caption (string) to word ids.
        tokens = nltk.tokenize.word_tokenize(
            str(caption).lower().decode('utf-8', errors='ignore'))
        caption = []
        caption.append(vocab('<start>'))
        caption.extend([vocab(token) for token in tokens])
        caption.append(vocab('<end>'))
...
class PrecompSkipDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab):
        ...
        self.captions = np.load(loc+'%s_senvecs.npy' % data_split)
...
class PrecompSTDataset(data.Dataset):
    """
    Load precomputed captions and image features
    Possible options: coco with skipthought caption vectors
    """
...
def collate_fn2(data):
    ...
    return images, targets, img_ids, ids
...
def get_loader_single(data_name, split, root, json, vocab, transform,
    ...
    if 'coco' in data_name:
        ...
        dataset = FlickrDataset(root=root,
                                ...
                                vocab=vocab,
                                transform=transform)
    elif 'minicsdv2' in data_name:
        dataset = MiniCSDV2Dataset(root=root,
                                    json=json,
                                    vocab=vocab,
                                    transform=transform, ids=ids)
...
def get_precomp_skip_loader(data_path, data_split, vocab, opt, batch_size=100,
                       shuffle=True, num_workers=2):
    """Returns torch.utils.data.DataLoader for custom coco dataset."""
    dset = PrecompSkipDataset(data_path, data_split, vocab)
...
def get_st_precomp_loader(data_path, data_split, vocab, opt, batch_size=100,
                       shuffle=True, num_workers=2):
    """Returns torch.utils.data.DataLoader for custom coco dataset."""
    dset = PrecompSTDataset(data_path, data_split, vocab)
...
def get_loaders(data_name, vocab, crop_size, batch_size, workers, opt):
    ...
    if opt.data_name == 'coco_st_precomp' or opt.data_name == 'coco_st_ner_precomp' or opt.data_name=='csd_ner_precomp' or opt.data_name=='breakingnews_precomp':
        train_loader  =get_st_precomp_loader(dpath, 'train', vocab, opt, batch_size, True, workers)

        val_loader = get_st_precomp_loader(dpath, 'dev', vocab, opt, batch_size, False, workers)

    elif opt.data_name == 'csd_skip_precomp':
        train_loader = get_precomp_skip_loader(dpath, 'dev', vocab, opt, batch_size, True, workers)

        val_loader = get_precomp_skip_loader(dpath, 'dev', vocab, opt, batch_size, False, workers)
...
def get_test_loader(split_name, data_name, vocab, crop_size, batch_size,
    ...
    if data_name=='coco_st_precomp' or data_name=='coco_st_ner_precomp' or data_name=='csd_ner_precomp' or opt.data_name=='breakingnews_precomp':
        test_loader = get_st_precomp_loader(dpath, 'test', vocab, opt, batch_size, False, workers)
    else:
